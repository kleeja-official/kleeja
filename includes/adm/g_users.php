...
switch ($current_smt):
    ...
    case 'group_acl':
        ...
        while ($row = $SQL->fetch_array($result)) {
            ...
            $acls[] = [
                'acl_title' => !empty($lang['ACLS_' . strtoupper($row['acl_name'])])
                    ? $lang['ACLS_' . strtoupper($row['acl_name'])]
                    : $olang['ACLS_' . strtoupper($row['acl_name'])],
    ...
    case 'group_data':
        ...
        $STAMP_IMG_URL = file_exists(PATH . 'images/watermark.gif')
            ...
            $data[] = [
                'option' => str_replace(
                    ...
                    '<div class="form-group">' .
                        ...
                        $row['name'] .
                        '">' .
                        (!empty($lang[strtoupper($row['name'])])
                            ? $lang[strtoupper($row['name'])]
                            : $olang[strtoupper($row['name'])]) .
                        '</label>' .
                        "\n" .
                        '<div class="box">' .
                        (empty($row['option'])
                            ? ''
                            : $tpl->admindisplayoption(
                                preg_replace(
                                    ['!{con.[a-z0-9_]+}!', '!NAME="con.!'],
                                    ['{cdata.' . $row['name'] . '}', 'NAME="cdata.'],
                                    $row['option'],
                                ),
                            )) .
                        '</div>' .
    ...
    case 'group_exts':
        ...
        while ($row = $SQL->fetch_array($result)) {
            ...
            $exts[] = [
                ...
                'ext_icon' => file_exists(PATH . 'images/filetypes/' . $row['ext'] . '.png')
                    ? PATH . 'images/filetypes/' . $row['ext'] . '.png'
                    : PATH . 'images/filetypes/file.png',
            ];
        }
        $SQL->freeresult($result);
    ...
    case 'users':
        ...
        if ($nums_rows > 0) {
            ...
            while ($row = $SQL->fetch_array($result)) {
                $userfile =
                    $config['siteurl'] .
                    ($config['mod_writer']
                        ? 'fileuser-' . $row['id'] . '.html'
                        : 'ucp.php?go=fileuser&amp;id=' . $row['id']);
        ...
        $page_nums = $Pager->print_nums(
            basename(ADMIN_PATH) .
                '?cp=' .
                basename(__FILE__, '.php') .
                (ig('search_id') ? '&search_id=' . g('search_id') : '') .
                (ig('qg') ? '&qg=' . g('qg', 'int') : '') .
                (ig('smt') ? '&smt=' . $current_smt : ''),
        );
...
if (ip('submit')) {
    $g_link =
        ...
        basename(__FILE__, '.php') .
        '&amp;page=' .
        (ig('page') ? g('page', 'int') : 1) .
        (ig('search_id') ? '&amp;search_id=' . g('search_id') : '') .
        '&amp;smt=' .
        $current_smt;

    $text =
        ($affected ? $lang['USERS_UPDATED'] : $lang['NO_UP_CHANGE_S']) .
