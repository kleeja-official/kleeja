...
switch ($case):
    ...
    case 'store':
        ...
        foreach ($store_catalog as $style_info) {
            ...
            $store_styles[$style_info['name']] = [
                ...
                'title' => !empty($style_info['title'][$config['language']])
                    ? $style_info['title'][$config['language']]
                    : $style_info['title']['en'],
                'website' => $style_info['website'],
                'current_version' => !empty($available_styles[$style_info['name']])
                    ? strtolower($available_styles[$style_info['name']]['info']['version'])
                    : '',
    ...
    case 'select':
        ...
        if (($style_info = kleeja_style_info($style_name)) != false) {
            ...
            if (
                isset($style_info['kleeja_version']) &&
                version_compare(strtolower($style_info['kleeja_version']), strtolower(KLEEJA_VERSION), '>')
            ) {
    ...
    case 'download':
        ...
        if (file_exists(PATH . 'styles/' . $style_name)) {
            ...
            if (!rename(PATH . 'styles/' . $style_name, PATH . 'styles/' . $style_name . '_backup')) {
                if (file_exists(PATH . 'styles/' . $style_name)) {
                        ...
                        if (file_exists(PATH . 'cache/' . $style_name . '.zip')) {
                            ...
                            if ($zip->open(PATH . 'cache/' . $style_name . '.zip') === true) {
                                if ($zip->extractTo(PATH . 'styles')) {
                                    ...
                                    $adminAjaxContent =
                                        '1:::' .
                                        sprintf($lang[$is_update ? 'ITEM_UPDATED' : 'ITEM_DOWNLOADED'], $style_name);
                                    ...
                                    $adminAjaxContent =
                                        '1003:::' . sprintf($lang['EXTRACT_ZIP_FAILED'], PATH . 'styles');
                                }
                            }
                        } else {
        ...
        if (strpos($adminAjaxContent, '1:::') === false) {
            if (file_exists(PATH . 'styles/' . $style_name . '_backup')) {
                rename(PATH . 'styles/' . $style_name . '_backup', PATH . 'styles/' . $style_name);
            }
        }
