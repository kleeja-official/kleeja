...
elseif ($current_smt == 'update2') {
    ...
    if ($zip->open(PATH . "cache/kleeja-{$new_version}.zip") == true) {
        ...
        if (rename(PATH . "cache/{$ex_folder}", PATH . "cache/kleeja-{$new_version}") === false) {
            copy(PATH . "cache/{$ex_folder}", PATH . "cache/kleeja-{$new_version}");
...
elseif ($current_smt == 'update3') {
    ...
    foreach ($files as $file) {
        if ($file->isFile()) {
            ...
            $backup->addFromString($file_path, file_get_contents($file_path));

            //copy file
            if (file_put_contents($file_path, file_get_contents($file->getPathname())) === false) {
