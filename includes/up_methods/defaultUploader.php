...
class defaultUploader implements KleejaUploader
{
    ...
    public function generateOutputBox(array $fileInfo): void
    {
        ...
        if ($is_img) {
            ...
            if (strpos($config['thmb_dims'], '*') !== false) {
                [$thmb_dim_w, $thmb_dim_h] = array_map('trim', explode('*', $config['thmb_dims']));
            }
        ...
        for ($i = 0; $i <= $this->getUploadFieldsLimit(); $i++) {
            ...
            if (empty($_FILES['file_' . $i . '_']['tmp_name']) && empty($_FILES['file'][$i]['tmp_name'])) {
                ...
                $filename = isset($_FILES['file'][$i]['name'])
                    ? $_FILES['file'][$i]['name']
                    : (isset($_FILES['file_' . $i . '_']['name'])
                        ? $_FILES['file_' . $i . '_']['name']
                        : '....');
    ...
    public function uploadTypeFile(int $fieldNumber, string $current_uploading_folder, int $current_user_id): void
    {
        ...
        $fileInfo['generatedFileName'] = change_filename_templates(
            trim($config['prefixname']) . $fileInfo['generatedFileName'],
        );
            ...
            if ($current_user_id == '-1') {
                $this->addErrorMessage(
                    sprintf($lang['FORBID_EXT'], $fileInfo['fileExtension']) .
                        '<br> <a href="' .
                        ($config['mod_writer'] ? 'register.html' : 'ucp.php?go=register') .
                        '" title="' .
        ...
        // check file size
        elseif (
            $this->getAllowedFileExtensions()[$fileInfo['fileExtension']] > 0 &&
