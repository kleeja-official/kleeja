...
switch (g('go')) {
    ...
    case 'login':
        ...
        if ($usrcp->name()) {
            ...
            $text =
                ...
                ' ..<br /> <a href="' .
                $config['siteurl'] .
                ($config['mod_writer'] ? 'logout.html' : 'ucp.php?go=logout') .
                '">' .
                $lang['LOGOUT'] .
                '</a>';
            kleeja_info($text);
    ...
    case 'register':
        ...
        if (!ip('submit')) {
            ...
            if (
                strlen(trim(p('lname'))) < 3 ||
                strlen(trim(p('lname'))) > 50 ||
                preg_match('/[^\p{L}_-]/u', p('lname'))
            ) {
    ...
    case 'fileuser':
        ...
        $your_fileuser =
            $config['siteurl'] .
            ($config['mod_writer']
                ? 'fileuser-' . $usrcp->id() . '.html'
                : 'ucp.php?go=fileuser&amp;id=' . $usrcp->id());
        ...
        if ($nums_rows != 0) {
            ...
            while ($row = $SQL->fetch_array($result)) {
                ...
                $file_info = [
                    ...
                    '::NAME::' => $row['name'],
                    '::DIR::' => $row['folder'],
                    '::FNAME::' => $row['real_filename'],
                ];

                $is_image = in_array(strtolower(trim($row['type'])), ['gif', 'jpg', 'jpeg', 'bmp', 'png'])
                    ? true
                    : false;
                ...
                $url_fileuser = $is_image
                    ? $url
                    : (file_exists('images/filetypes/' . $row['type'] . '.png')
            ...
            if (ip('submit_all_files') && $user_himself) {
                if (isset($ids) && !empty($ids)) {
                    ...
                    $log_msg =
                        $usrcp->name() .
                        ' has deleted all his/her files at this time : ' .
                        date('H:i a, d-m-Y') .
                        "] \r\n" .
                        'files numbers:' .
                        $files_num .
    ...
    case 'profile':
        ...
        if (ip('submit_data')) {
            ...
            if (!empty(p('ppass_new'))) {
                ...
                elseif (
                    empty(p('ppass_old')) ||
                    !$usrcp->kleeja_hash_password(p('ppass_old') . $password_salt, $userinfo['password'])
                ) {
            ...
            if ($usrcp->mail() != trim(strtolower(p('pmail')))) {
                ...
                if (
                    empty(p('pppass_old')) ||
                    !$usrcp->kleeja_hash_password(p('pppass_old') . $password_salt, $userinfo['password'])
                ) {
                    $ERRORS['curnt_old_pass'] = $lang['CURRENT_PASS_WRONG'];
                }
    ...
    case 'get_pass':
        ...
        if (ig('activation_key') && ig('uid')) {
            ...
            if ($SQL->num_rows($result)) {
                ...
                $text =
                    $lang['OK_APPLY_NEWPASS'] .
                    '<br /><a href="' .
                    $config['siteurl'] .
                    ($config['mod_writer'] ? 'login.html' : 'ucp.php?go=login') .
                    '">' .
                    $lang['LOGIN'] .
                    '</a>';
                kleeja_info($text);
